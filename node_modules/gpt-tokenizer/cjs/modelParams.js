...
function O200KBase(mergeableBytePairRanks) {
    const specialTokenMapping = new Map([
        ...
        [specialTokens_js_1.FimPrefix, 200000],
        [specialTokens_js_1.FimMiddle, 200001],
        [specialTokens_js_1.FimSuffix, 200002],
        [specialTokens_js_1.ImStart, 200003],
        [specialTokens_js_1.ImEnd, 200004],
        [specialTokens_js_1.ImSep, 200005],
        [specialTokens_js_1.EndOfPrompt, 200006],
    ]);
    return {
        tokenSplitRegex: /(?:'s|'t|'re|'ve|'m|'ll|'d)|[^\r\n\p{L}\p{N}]?\p{L}+|\p{N}{1,3}| ?[^\s\p{L}\p{N}]+[\r\n]*|\s*[\r\n]+|\s+(?!\S)|\s+/giu,
        mergeableBytePairRanks,
